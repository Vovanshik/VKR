...
class PDNScannerGUI(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.table.setHorizontalHeaderLabels(
            ["✔", "Полный путь", "Имя файла", "Владелец", "Типы ПДн", "Срок документа"]
        )
        ...
        keywords_btn = QtWidgets.QPushButton("Загрузить пользовательские слова")
        ...
        for row in range(self.table.rowCount()):
            ...
            for col in range(self.table.columnCount()):
                ...
                if highlight and is_old:
                    item.setBackground(QtGui.QColor("#FFE4E1"))  # светло-красный
                else:
    ...
    def fill_table(self, results):
        ...
        for item in results:
            ...
            types_list = types_list.replace("ADDRESS","Адресс")
            types_list = types_list.replace("BIRTHDATE", "ДатаРождения")
            types_list = types_list.replace("SNILS", "СНИЛС")
            types_list = types_list.replace("PER", "ФамИмяОтч")
            types_list = types_list.replace("INN", "ИНН")
            types_list = types_list.replace("ADDRESS", "Адресс")
            ...
            #владелец
            self.table.setItem(row, 3, QtWidgets.QTableWidgetItem(owner))

            #типы пдн
            self.table.setItem(row, 4, QtWidgets.QTableWidgetItem(types_list))

            #возраст
            self.table.setItem(row, 5, QtWidgets.QTableWidgetItem(age))
    ...
    def load_keywords(self):
        ...
        path, _ = QtWidgets.QFileDialog.getOpenFileName(
            self, "Файл с пользов. словами", "", "Text files (*.txt)"
        )
        if not path:
            return
        ...
        self.status_label.setText(
            f"Загружено пользов. слов: {len(self.custom_keywords)}"
        )
