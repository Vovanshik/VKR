...
print("Загрузка моделей Natasha... Это может занять некоторое время.")
segmenter = Segmenter()
morph_vocab = MorphVocab()
emb = NewsEmbedding()
ner_tagger = NewsNERTagger(emb)
print("Модели Natasha успешно загружены.")
...
def faceless_documents(item):
    ...
    if extension == ".txt":
        ...
        with open(out_path, "w", encoding="utf-8") as f:
            f.write(masked_text)
        print(f"[TXT] Обезличено → {out_path}")
        return
    ...
    if extension == ".docx":
        try:
            ...
            doc.save(out_path)
            print(f"[DOCX] Обезличено → {out_path}")
        except Exception as e:
...
if __name__ == "__main__":
    TARGET_DIRECTORY = "C:/Users/vovam/Desktop/ПО/Доки/Смесь"
    ...
    if os.path.isdir(TARGET_DIRECTORY):
        ...
        print("\n" + "=" * 50)
        print(f"Всего файлов с находками: {len(final_results)}")
        print("Обезличить данные? (да/нет)")
        answer = input("")
        if answer.lower() == "да":
            faceless_text(final_results)
        else:
            print("Работа завершена")
        print("Переместить найденные файлы в архив? (да/нет)")
        answer = input("")
        if answer.lower() == "да":
        ...
        print(f"Ошибка: Указанный путь не является директорией или не существует: {TARGET_DIRECTORY}")
